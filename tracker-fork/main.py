...
search_icon = icon_path('search')
open_icon = icon_path('document-open')
...
#                command = ['tracker', 'sparql', '-q', "SELECT nfo:fileName(?f) nie:url(?f) WHERE { ?f nie:url ?url FILTER(fn:starts-with(?url, \'file://" + home + "/\')) . ?f nie:plainTextContent ?w FILTER regex(?w, '"+query_words+"', 'i') }"]
                ...
                if '-a' in words:
                    words.remove('-a')
                    query_words = query_words.replace(' -a', '')
                    keys.append('-a')
                if '-r' in words:
                    words.remove('-r')
                    query_words = query_words.replace(' -r', '')
                    keys.append('-r')
                if '-c' in words:
                    words.remove('-c')
                    query_words = query_words.replace(' -c', '')
                    keys.append('-c')
                if len(words) == 1:
                    if '-c' in keys:
                ...
                pre_results = output.splitlines()
                if not '-a' in keys:
                    pre_results = [i for i in pre_results if (not '/.' in i)]
                if not '-r' in keys:
                    pre_results = [i for i in pre_results if i[0:len(home)] == home]
            ...
            for i in results:
                ...
                if os.path.isdir(data):
                    ...
                    content_type, val = Gio.content_type_guess('filename=' + data, data=None)
                    types = Gio.content_type_get_icon(content_type).to_string().split(' ')[2:]
                    mime_path = icon_path(*types)
                items.append(ExtensionResultItem(icon=mime_path,
                                                 name='%s' %i[0],
                                                 description="%s" %i[1],
                                                 on_enter=ExtensionCustomAction(data, keep_app_open=True)))
